...
export default function PartnerSheetPage() {
  ...
  const handleAddProduct = (data: { optionName: string; popupContent: string; prices: Record<string, number>; cellValues: Record<string, string> }) => {
    setRows((prev) => [...prev, {
      id: '',
      sheetId: sheet?.id || '',
  ...
  const handleEditProduct = (index: number, data: { optionName: string; popupContent: string; prices: Record<string, number>; cellValues: Record<string, string> }) => {
