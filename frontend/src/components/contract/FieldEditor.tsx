...
export default function FieldEditor({ fields, onChange, templateImageUrl }: FieldEditorProps) {
  ...
  const btnClass = "flex flex-col items-center gap-0.5 p-1.5 rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-200 transition-colors text-gray-600 hover:text-blue-600";
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {FIELD_TYPES.map(({ value, label, icon: Icon }) => (
              <button
                ...
                className="flex items-center gap-1.5 px-2 py-1.5 text-xs rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-200 transition-colors"
              >
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          ...
          {selectedIndices.size >= 2 && (
            <div className="flex items-center gap-1 mb-2 p-1.5 bg-blue-50 rounded-lg">
              ...
              <button onClick={removeSelectedFields} className="p-1 rounded hover:bg-error-light text-error" title="삭제"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          )}
              ...
              fields.map((field, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button onClick={(e) => { e.stopPropagation(); removeField(idx); }} className="text-gray-400 hover:text-error"><Trash2 className="w-3 h-3" /></button>
                  </div>
                );
              })
            )}
